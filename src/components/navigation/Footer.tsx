import { getTranslations } from "next-intl/server"
import { Mail, ArrowUpRight } from "lucide-react"

import { Link } from "@/i18n/navigation"
import { Container } from "@/components/layout/Container"

function WhatsAppIcon(props: React.SVGProps<SVGSVGElement>) {
	return (
		<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" {...props}>
			<path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.669.15-.198.297-.768.967-.942 1.165-.173.198-.347.223-.644.074-.297-.148-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.372-.025-.521-.075-.148-.669-1.612-.916-2.206-.242-.579-.487-.5-.669-.51-.173-.01-.372-.01-.571-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.095 3.2 5.076 4.487.709.306 1.262.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884a9.83 9.83 0 0 1 6.982 2.893 9.825 9.825 0 0 1 2.895 6.983c-.002 5.45-4.437 9.884-9.89 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05.003C5.495.003.16 5.338.157 11.892c0 2.096.547 4.142 1.588 5.945L0 24l6.305-1.654a11.867 11.867 0 0 0 5.741 1.468h.005c6.554 0 11.89-5.335 11.893-11.89a11.82 11.82 0 0 0-3.48-8.413Z" />
		</svg>
	);
}


export async function Footer() {
	const [t, data] = await Promise.all([
		getTranslations("Footer"),
		getTranslations("Data")
	])

	const year = new Date().getFullYear()

	const companyName = data("brandFull")
	const email = data("email")
	const whatsappHref = data("whatsappHref")
	const authorUrl = data("authorUrl")
	const authorName = data("authorName")

	const nav = [
		{ href: "/#hero", label: t("nav.hero") },
		{ href: "/#products", label: t("nav.products") },
		{ href: "/#industries", label: t("nav.industries") },
		{ href: "/#coverage", label: t("nav.coverage") },
		{ href: "/#providers", label: t("nav.providers") },
		{ href: "/#testimonials", label: t("nav.testimonials") },
		{ href: "/#allies", label: t("nav.allies") },
		{ href: "/#contact", label: t("nav.contact") }
	]

	return (
		<footer className="border-t">
			<Container className="py-10">
				<div className="grid gap-8 md:grid-cols-3 md:items-start">
					<div className="space-y-3">
						<Link href="/#hero" className="inline-flex items-center gap-2">
							<span className="font-display text-base font-semibold tracking-tight">
								{companyName}
							</span>
							<ArrowUpRight className="h-4 w-4 text-muted-foreground" />
						</Link>

						<p className="text-sm text-muted-foreground">{t("tagline")}</p>
						<p className="text-xs text-muted-foreground">{t("builtFor")}</p>
					</div>

					<div className="space-y-3">
						<p className="text-sm font-medium text-foreground/90">{t("sections")}</p>
						<ul className="grid grid-cols-2 gap-2 text-sm">
							{nav.map((item) => (
								<li key={item.href}>
									<Link
										href={item.href}
										className="text-muted-foreground transition-colors hover:text-foreground"
									>
										{item.label}
									</Link>
								</li>
							))}
						</ul>
					</div>

					<div className="space-y-3">
						<p className="text-sm font-medium text-foreground/90">{t("connect")}</p>

						<ul className="space-y-2 text-sm">
							<li>
								<a
									href={`mailto:${email}`}
									className="inline-flex items-center gap-2 text-muted-foreground transition-colors hover:text-foreground"
								>
									<Mail className="h-4 w-4 text-primary" />
									{t("social.email")}
								</a>
							</li>

							<li>
								<a
									href={whatsappHref}
									target="_blank"
									rel="noreferrer"
									className="inline-flex items-center gap-2 text-muted-foreground transition-colors hover:text-foreground"
								>
									<WhatsAppIcon className="h-4 w-4 text-primary" />
									{t("social.whatsapp")}
								</a>
							</li>

							<li>
								<Link
									href="/#contact"
									className="inline-flex items-center gap-2 text-muted-foreground transition-colors hover:text-foreground"
								>
									<ArrowUpRight className="h-4 w-4 text-primary" />
									{t("social.quote")}
								</Link>
							</li>
						</ul>

						<div className="pt-2">
							<Link
								href="/#hero"
								className="inline-flex items-center gap-2 text-xs text-muted-foreground transition-colors hover:text-foreground"
							>
								{t("backToTop")}
								<ArrowUpRight className="h-3.5 w-3.5" />
							</Link>
						</div>
					</div>
				</div>

				<div className="mt-10 flex flex-col gap-2 border-t pt-6 sm:flex-row sm:items-center sm:justify-between">
					<p className="text-xs text-muted-foreground">
						{t("copyright", { year, name: companyName })}
					</p>

					<p className="text-xs text-muted-foreground">
						{t.rich("credit", {
							name: authorName,
							author: (chunks) => (
								<a href={authorUrl} target="_blank" rel="noreferrer" className="underline underline-offset-4">
									{chunks}
								</a>
							)
						})}
					</p>
				</div>
			</Container>
		</footer>
	)
}
